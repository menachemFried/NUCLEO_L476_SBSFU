...
# ==============================================================================
#
# POST-BUILD SCRIPT for SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256
#
...
# --- 3. SCRIPT BODY ---
info_log $LINENO "--- Post-Build Script Started (ECCDSA_WITH_AES) ---"
...
OEM_KEY_ABS="${COMMON_ABS_DIR}/Binary_Keys/OEM_KEY_COMPANY${FW_ID}_key_AES_CBC.bin"
debug_log $LINENO "OEM Key File (Absolute): \"$OEM_KEY_ABS\""
IV_FILE_ABS="${COMMON_ABS_DIR}/Binary_Keys/iv.bin"
debug_log $LINENO "IV File (Absolute): \"$IV_FILE_ABS\""
PARTIAL_BIN_ABS="${BINARY_OUTPUT_DIR_ABS}/Partial${EXEC_NAME}.bin"
debug_log $LINENO "Partial BIN File (Absolute): \"$IV_FILE_ABS\""
...
if [ $ret -eq 0 ]; then
  ...
  debug_log $LINENO "UNIX-formatted OEM Key : \"$OEM_KEY_UNIX\""
  ...
  IV_FILE_UNIX=$(echo "$IV_FILE_ABS" | sed 's/\\/\//g')
  debug_log $LINENO "UNIX-formatted IV File: \"$IV_FILE_UNIX\""
  SBSFU_ELF_UNIX=$(echo "$SBSFU_ELF_ABS" | sed 's/\\/\//g')
  debug_log $LINENO "UNIX-formatted SBSFU ELF File: \"$SBSFU_ELF_UNIX\""

  info_log $LINENO "1. Encrypting binary with AES-CBC..."
  command="$PREPARE_IMAGE_CMD_SCRIPT enc -k \"$OEM_KEY_UNIX\" -i \"$IV_FILE_UNIX\" \"$BIN_FILE_UNIX\" \"$SFU_FILE_UNIX\""
  debug_log $LINENO "EXECUTING: $command"
  "$@" enc -k "$OEM_KEY_UNIX" -i "$IV_FILE_UNIX" "$BIN_FILE_UNIX" "$SFU_FILE_UNIX" > "$OUTPUT_DIR"/output.txt
  ret=$?
if [ $ret -eq 0 ]; then
  info_log $LINENO "2. Signing encrypted binary with ECCDSA..."
  ...
  ret=$?
    if [ $ret -eq 0 ]; then
      ...
      command="$PREPARE_IMAGE_CMD_SCRIPT pack -m \"$MAGIC\" -k \"$SIGN_KEY_UNIX\" -r 28 -v \"$VERSION\" -i \"$IV_FILE_UNIX\" -f \"$SFU_FILE_UNIX\" -t \"$SIGN_FILE_UNIX\" \"$SFB_FILE_UNIX\" -o \"$OFFSET\""
      debug_log $LINENO "EXECUTING: $command"
      "$@" pack -m "$MAGIC" -k "$SIGN_KEY_UNIX" -r 28 -v "$VERSION" -i "$IV_FILE_UNIX" -f "$SFU_FILE_UNIX" -t "$SIGN_FILE_UNIX" "$SFB_FILE_UNIX" -o "$OFFSET" >> "$OUTPUT_DIR"/output.txt
      ret=$?
      if [ $ret -eq 0 ]; then
        info_log $LINENO "4. Creating header binary..."
        command="$PREPARE_IMAGE_CMD_SCRIPT header -m \"$MAGIC\" -k \"$SIGN_KEY_UNIX\" -r 28 -v \"$VERSION\" -i \"$IV_FILE_UNIX\" -f \"$SFU_FILE_UNIX\" -t \"$SIGN_FILE_UNIX\" -o \"$OFFSET\" \"$HEADER_BIN_UNIX\""
        debug_log $LINENO "EXECUTING: $command"
        "$@" header -m "$MAGIC" -k "$SIGN_KEY_UNIX" -r 28 -v "$VERSION" -i "$IV_FILE_UNIX" -f "$SFU_FILE_UNIX" -t "$SIGN_FILE_UNIX" -o "$OFFSET" "$HEADER_BIN_UNIX" >> "$OUTPUT_DIR"/output.txt
        ret=$?
        if [ $ret -eq 0 ]; then
          ...
          if [ $ret -eq 0 ]; then
            ...
            if [ -f "$REF_USER_APP_ABS" ]; then
              ...
              if [ $ret -eq 0 ]; then
                ...
                command="$PREPARE_IMAGE_CMD_SCRIPT enc -k \"$OEM_KEY_UNIX\" -i \"$IV_FILE_UNIX\" \"$PARTIAL_BIN_UNIX\" \"$PARTIAL_SFU_UNIX\""
                debug_log $LINENO "EXECUTING: $command"
                "$@" enc -k "$OEM_KEY_UNIX" -i "$IV_FILE_UNIX" "$PARTIAL_BIN_UNIX" "$PARTIAL_SFU_UNIX" >> "$OUTPUT_DIR"/output.txt
                ret=$?
                if [ $ret -eq 0 ]; then
                  info_log $LINENO "6c. Signing partial encrypted binary..."
                  command="$PREPARE_IMAGE_CMD_SCRIPT sha256 \"$PARTIAL_BIN_UNIX\" \"$PARTIAL_SIGN_UNIX\""
                  debug_log $LINENO "EXECUTING: $command"
                  "$@" sha256 "$PARTIAL_BIN_UNIX" "$PARTIAL_SIGN_UNIX" >> "$OUTPUT_DIR"/output.txt
                  ret=$?
                  if [ $ret -eq 0 ]; then
                    info_log $LINENO "6d. Packing partial SFB..."
                    command="$PREPARE_IMAGE_CMD_SCRIPT pack -m \"$MAGIC\" -k \"$SIGN_KEY_UNIX\" -r 28 -v \"$VERSION\" -i \"$IV_FILE_UNIX\" -f \"$SFU_FILE_UNIX\" -t \"$SIGN_FILE_UNIX\" -o \"$OFFSET\" --pfw \"$PARTIAL_SFU_UNIX\" --ptag \"$PARTIAL_SIGN_UNIX\" --poffset \"$PARTIAL_OFFSET_UNIX\" \"$PARTIAL_SFB_UNIX\""
                    debug_log $LINENO "EXECUTING: $command"
                    "$@" pack -m "$MAGIC" -k "$SIGN_KEY_UNIX" -r 28 -v "$VERSION" -i "$IV_FILE_UNIX" -f "$SFU_FILE_UNIX" -t "$SIGN_FILE_UNIX" -o "$OFFSET" --pfw "$PARTIAL_SFU_UNIX" --ptag "$PARTIAL_SIGN_UNIX" --poffset "$PARTIAL_OFFSET_UNIX" "$PARTIAL_SFB_UNIX" >> "$OUTPUT_DIR"/output.txt
                    ret=$?
                  fi
                fi
              fi
